      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href={`/${locale}`} className="flex items-center gap-2">
            <span className="text-primary text-2xl font-black tracking-tight font-english">Q8</span>
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-silver hover:text-primary transition-colors text-sm font-medium"
              >
                {link.label}
              </Link>
            ))}
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div className="md:hidden pb-4 border-t border-metal mt-2 pt-4">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                onClick={() => setMenuOpen(false)}
                className="block py-2 text-silver hover:text-primary transition-colors"
              >
                {link.label}
              </Link>
            ))}
            <div className="flex items-center gap-3 mt-4 pt-4 border-t border-metal">
              ...
              {user ? (
                <>
                  ...
                  <button onClick={logout} className="text-silver hover:text-primary text-sm">
                    {t.common.logout}
                  </button>
                </>
              ) : (
