  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-5 pb-20 sm:px-8">
      ...
      <section className="mt-12">
        ...
        <div className="mt-6 grid gap-4 min-[380px]:grid-cols-2 sm:gap-5 lg:grid-cols-3">
          ...
          {partListings.slice(0, 6).map((part) => (
            ...
            >
              <div className="relative h-40 bg-metal min-[380px]:h-28 sm:h-40">
                ...
                <div className="absolute bottom-3 left-3 rounded-full bg-black/60 px-3 py-1.5 text-[11px] font-extrabold tracking-wide text-white backdrop-blur-sm">
                  👁️ {(part.views ?? 0).toLocaleString("ar-KW")} مشاهدة
                </div>
                {part.featuredAt ? (
                  <span className="absolute top-2 left-2 rounded-full border-2 border-gold bg-background/60 px-2.5 py-1 text-[10px] font-black text-gold backdrop-blur-sm sm:top-3 sm:left-3 sm:px-3 sm:text-[11px]">
                    إعلان مميز
              ...
              <div className="p-4 min-[380px]:p-3 sm:p-4">
                ...
                <p className="mt-3 text-base font-black text-brand sm:text-lg">{part.price}</p>
              </div>
            </Link>
          ))}
        </div>
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-black text-foreground sm:text-xl">{icon} {title}</h2>
      </div>
